...
def compute_delays(read_content, translate_content, src_lang, tgt_lang):
    ...
    if not delays:
        # Degenerate case: every WRITE step produced zero actual translated
        # tokens (e.g. the model emitted only the end-of-write token itself,
        # content-free -- seen from a severely undertrained checkpoint, but
        # possible from any model on a pathological input). Treat it as
        # maximal lag -- "read everything, said nothing" -- the same sentinel
        # AverageLagging/LengthAdaptiveAverageLagging already special-case via
        # "delays[0] > source_length" -- instead of crashing the whole eval
        # sweep over one degenerate sample.
        delays = [cumulative_lengths[-1] if cumulative_lengths else 0]
